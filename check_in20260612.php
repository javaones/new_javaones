...
<body>
  ...
  <div class="container">
    ...
    <div class="export-wrap">
      <button class="export-btn" id="exportBtn">📗 导出所有教师（Excel）</button>
      <button class="export-btn secondary" id="exportGradeBtn">📘 按年级导出（Excel）</button>
    </div>
  </div>
  ...
  <script>
    ...
    async function exportMonthSummary() {
      return exportWorkbookByTeachers(state.teachers, '导出所有教师');
    }
    ...
    async function exportByGrade() {
      ...
      return exportWorkbookByTeachers(teachers, state.selectedGrade || '按年级导出');
    }

    async function exportWorkbookByTeachers(teachers, scopeLabel) {
      if (typeof XLSX === 'undefined') {
        alert('Excel 导出库加载失败，请检查网络或使用支持 CDN 的环境。');
        return;
      }
      ...
      const headerRow1 = ['姓名', '班级'];
      const headerRow2 = [null, null];
      let col = 2;

      for (let d = 1; d <= days; d++) {
        const dt = new Date(year, month - 1, d);
        const label = `${year}/${month}/${d}（周${weekdayText(dt)}）`;
        headerRow1.push(label, null);
        headerRow2.push('第7节', '第8节');
      ...
      rows.push([`${scopeLabel} - ${monthKey}`, '', ...Array(days * 2 + 1).fill('')]);
      ...
      ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 2 + days * 2 } }, ...merges];
      ws['!cols'] = [{ wch: 12 }, { wch: 10 }];
      for (let i = 0; i < days * 2; i++) ws['!cols'].push({ wch: 8 });
      ws['!cols'].push({ wch: 10 });

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
      const fileName = `${scopeLabel}_${monthKey}.xlsx`;
